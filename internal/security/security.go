...
func (ss *SecurityService) getPipHashes(ctx context.Context, packageName, version string) (map[string]string, error) {
	...
	hashes := make(map[string]string)
	if len(data.URLs) > 0 {
		for algo, hash := range data.URLs[0].Digests {
			hashes[algo] = hash
		}
	}
...
func (ss *SecurityService) checkVulnerabilityDatabases(ctx context.Context, packageName, version, packageType string) ([]SecurityCheck, error) {
	...
	if err != nil {
		...
		logger.Warn("Failed to query OSV: %v", err)
	} else {
	...
	if err != nil {
		...
		logger.Warn("Failed to query GitHub Advisories: %v", err)
	} else {
...
func (ss *SecurityService) isTyposquattingCandidate(candidate, target string) bool {
	...
	dist := ss.levenshtein(candidate, target)

	// If distance is small (1 or 2) relative to string length, consider it typosquatting
	if dist > 0 && dist <= 2 {
...
func (ss *SecurityService) levenshtein(s1, s2 string) int {
