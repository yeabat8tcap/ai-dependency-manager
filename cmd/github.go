...
func runGitHubApprovalStatus(cmd *cobra.Command, args []string) error {
	...
	*/
	fmt.Println("Approval workflow status check not implemented yet")
...
func runGitHubWebhookList(cmd *cobra.Command, args []string) error {
	...
	// List webhooks
	webhooks, err := manager.GetWebhooks().List(ctx, parts[0], parts[1])
	if err != nil {
...
func runGitHubPatchList(cmd *cobra.Command, args []string) error {
	...
	if len(args) > 0 {
		...
		prs, err := manager.GetPullRequests().ListPatchPRs(ctx, parts[0], parts[1])
		if err != nil {
	...
	for _, repo := range repos {
		...
		prs, err := manager.GetPullRequests().ListPatchPRs(ctx, repo.Owner, repo.Name)
		if err != nil {
